...
from routes.goal_manifestation import verify_manifestation_access

@pytest.mark.asyncio
async def test_verify_manifestation_access_admin():
    user = {"user_id": "admin_1", "role": "super_admin", "user_type": "free", "subscription_plan": "none"}
    res = await verify_manifestation_access(user)
    assert res is True

@pytest.mark.asyncio
async def test_verify_manifestation_access_free_user():
    user = {"user_id": "free_user_1", "role": "user", "user_type": "free", "subscription_plan": "none"}
    with patch("routes.goal_manifestation.db") as mock_db:
        ...
        assert "Goal Manifestation is not available for Free or On-Demand plans" in exc_info.value.detail

@pytest.mark.asyncio
async def test_verify_manifestation_access_on_demand_user():
...
@pytest.mark.asyncio
async def test_verify_manifestation_access_basic_user():
    ...
    with patch("routes.goal_manifestation.db") as mock_db:
        ...
        with pytest.raises(HTTPException) as exc_info:
            await verify_manifestation_access(user)
        assert exc_info.value.status_code == 403
        assert "Basic or Pro plan users" in exc_info.value.detail

@pytest.mark.asyncio
async def test_verify_manifestation_access_pro_user():
...
@pytest.mark.asyncio
async def test_verify_manifestation_access_premium_user():
