...
@pytest.mark.asyncio
async def test_verify_ctt_access_admin():
    user = {"user_id": "admin_1", "role": "super_admin", "user_type": "free", "subscription_plan": "none"}
    res = await verify_ctt_access(user)
    assert res is True

@pytest.mark.asyncio
async def test_verify_ctt_access_free_user():
    ...
    with patch("routes.ctt_gem.db") as mock_db:
        ...
        assert "CTT (Task Tracker) is not available for Free or On-Demand plans" in exc_info.value.detail

@pytest.mark.asyncio
async def test_verify_ctt_access_on_demand_user():
...
@pytest.mark.asyncio
async def test_verify_ctt_access_subscribed_user():
