...
@pytest.mark.asyncio
async def test_verify_decision_templates_access_admin():
    user = {"user_id": "admin_1", "role": "super_admin", "user_type": "free", "subscription_plan": "none"}
    res = await verify_decision_templates_access(user)
    assert res is True

@pytest.mark.asyncio
async def test_verify_decision_templates_access_free_user():
    ...
    with patch("routes.decider_store.db") as mock_db:
        ...
        assert "Decision Templates (The Decider Store) are not available for Free or On-Demand plans" in exc_info.value.detail

@pytest.mark.asyncio
async def test_verify_decision_templates_access_on_demand_user():
    user = {"user_id": "ondemand_user_1", "role": "user", "user_type": "on_demand_l1", "subscription_plan": "on_demand_l1"}
    with patch("routes.decider_store.db") as mock_db:
        mock_db.app_settings.find_one = AsyncMock(return_value=None)
        mock_db.users.find_one = AsyncMock(return_value={"user_type": "on_demand_l1", "subscription_plan": "on_demand_l1"})
        mock_db.credit_wallets.find_one = AsyncMock(return_value=None)
...
@pytest.mark.asyncio
async def test_verify_decision_templates_access_basic_user():
...
@pytest.mark.asyncio
async def test_verify_decision_templates_access_pro_user():
