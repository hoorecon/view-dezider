...
async def update_feature_flags(request: Request, user: dict = Depends(get_current_user)):
    ...
    await db.app_settings.update_one(
        {"key": "feature_flags"},
        {"$set": {"flags": flags, "updated_at": datetime.now(timezone.utc).isoformat(), "updated_by": user["user_id"]}},
        upsert=True
    )
...
@api_router.post("/solution-finders")
async def create_solution_finder(request: Request, user: dict = Depends(get_current_user)):
    ...
    doc = {
        ...
        "milestones": body.get("milestones", []),  # [{description, timeline}]
        ...
        "action_items": body.get("action_items", []),  # [{action, who, by_when, status}]
        "status": body.get("status", "in_progress"),  # in_progress, completed
        "created_at": now,
        "updated_at": now
    }
...
async def create_solution_matrix(request: Request, user: dict = Depends(get_current_user)):
    ...
    doc = {
        ...
        # Simpler solution section (3.1.1)
        ...
        # Solution Matrix (3.1.2) - Self/Micro/Macro with 7 sub-areas each
        ...
        "q4_contingency_plans": body.get("q4_contingency_plans", ""),
        # Action Plan (Q5)
        "action_items": body.get("action_items", []),  # [{who, what, by_when, status}]
...
# ========================
# P1: ADMIN CALL CONFIG
# ========================

@api_router.get("/admin/call-config")
...
@api_router.put("/admin/call-config")
