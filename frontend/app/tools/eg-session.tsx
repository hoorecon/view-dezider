          ...
          {report ? (
            <View style={st.reportCard}>
              ...
              <Text style={st.disclaimer}>
                Disclaimer: This AI-generated report contains suggestions only and is NOT
                professional advice. JELCOS AI is not responsible for any actions taken
                based on it. Please consult relevant subject-matter experts (e.g. licensed
                therapists, financial advisors, legal counsel) before making decisions
                with material consequences.
              </Text>

              {/* Report actions — reuse decision-flow style: Download PDF,
                  Share Email, Share WhatsApp, Action Items Planner. */}
              <View style={st.reportActions}>
                <TouchableOpacity
                  testID="report-download-pdf-btn"
                  style={st.reportActionBtn}
                  onPress={() => {
                    if (Platform.OS === 'web' && typeof window !== 'undefined') {
                      window.print();
                    } else {
                      Alert.alert('Download PDF', 'On mobile, use Share → Save to Files (PDF) from the next dialog.');
                    }
                  }}>
                  <Ionicons name="download-outline" size={14} color="#7C3AED" />
                  <Text style={st.reportActionText}>Download PDF</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  testID="report-share-email-btn"
                  style={st.reportActionBtn}
                  onPress={() => {
                    const subj = encodeURIComponent(report.report_title || 'My Breakthrough Report');
                    const body = encodeURIComponent(
                      (report.sections || []).map((s: any) => `${s.title}\n${s.content}\n`).join('\n')
                      + '\n\n— Generated by JELCOS AI'
                    );
                    Linking.openURL(`mailto:?subject=${subj}&body=${body}`);
                  }}>
                  <Ionicons name="mail-outline" size={14} color="#7C3AED" />
                  <Text style={st.reportActionText}>Share Email</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  testID="report-share-whatsapp-btn"
                  style={st.reportActionBtn}
                  onPress={() => {
                    const txt = encodeURIComponent(
                      `${report.report_title || 'My Breakthrough Report'}\n\n`
                      + (report.sections || []).slice(0, 2).map((s: any) => `*${s.title}*\n${s.content}`).join('\n\n')
                      + '\n\n— Generated by JELCOS AI'
                    );
                    Linking.openURL(`https://wa.me/?text=${txt}`);
                  }}>
                  <Ionicons name="logo-whatsapp" size={14} color="#7C3AED" />
                  <Text style={st.reportActionText}>Share WhatsApp</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  testID="report-action-items-btn"
                  style={st.reportActionBtn}
                  onPress={() => router.push({
                    pathname: '/tools/action-center' as any,
                    params: { source: 'aim_session', session_id: sessionId },
                  })}>
                  <Ionicons name="list" size={14} color="#7C3AED" />
                  <Text style={st.reportActionText}>Action Items Planner</Text>
                </TouchableOpacity>
              </View>
            </View>
...
const st = StyleSheet.create({
  ...
  disclaimer: {
    ...
    fontSize: 10.5, color: '#6B7280', lineHeight: 14, fontStyle: 'italic',
  },
  reportActions: {
